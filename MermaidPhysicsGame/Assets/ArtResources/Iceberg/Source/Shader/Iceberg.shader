...
////// Specular:
                ...
                float3 diffuseColor = (lerp(((_MainTex_var.rgb*_BaseColor.rgb)*pow(_AO_var.rgb,_AOIntensity)),_DetailTexture_var.rgb,_DetailTextureintensity)+(_FresnelColor.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelRadius)*_Fresnelintensity)); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _AO_var = tex2D(_AO,TRANSFORM_TEX(i.uv0, _AO));
                float4 _DetailTexture_var = tex2D(_DetailTexture,TRANSFORM_TEX(i.uv0, _DetailTexture));
                float3 diffColor = (lerp(((_MainTex_var.rgb*_BaseColor.rgb)*pow(_AO_var.rgb,_AOIntensity)),_DetailTexture_var.rgb,_DetailTextureintensity)+(_FresnelColor.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelRadius)*_Fresnelintensity));
                float4 _BaseSpecular_var = tex2D(_BaseSpecular,TRANSFORM_TEX(i.uv0, _BaseSpecular));
